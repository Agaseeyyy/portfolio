                            ...
                            <tr>
                                    ...
                                    <div class="flex flex-wrap gap-1">
                                        <?php if (!empty($project['technologies'])): ?>
                                            <?php foreach (array_slice($project['technologies'], 0, 3) as $tech): ?>
                                                <span class="badge badge-primary badge-soft text-xs"><?= htmlspecialchars($tech['tech_name']) ?></span>
                                            <?php endforeach; ?>
                                            <?php if (count($project['technologies']) > 3): ?>
                                                <span class="badge badge-primary badge-soft text-xs">+<?= count($project['technologies']) - 3 ?></span>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <span class="badge badge-primary badge-soft text-xs">No tech</span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="flex justify-end gap-2">
                                        <button onclick="openEditModal(<?= htmlspecialchars(json_encode($project), ENT_QUOTES, 'UTF-8') ?>)" class="btn btn-ghost btn-sm">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </button>
                                        <button onclick="confirmDelete(<?= $project['project_id'] ?>, '<?= htmlspecialchars($project['project_name']) ?>')" class="btn btn-ghost btn-sm text-red-400 hover:bg-red-500/10">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="modal-box">
        <form id="projectForm" method="POST" enctype="multipart/form-data">
            ...
            <fieldset class="fieldset">
                <!-- Project Name -->
                <label class="modal-label">Project Name <span class="text-pink-400 text-xs right-0">Required</span></label>
                <input type="text" name="project_name" id="projectName" class="input" required>

                <!-- Role -->
                <label class="modal-label">Your Role</label>
                <input type="text" name="role" id="projectRole" class="input" placeholder="e.g., Full-stack Developer, UI Designer">

                <!-- Description -->
                <label class="modal-label">Short Description</label>
                <textarea name="description" id="projectDescription" rows="2" class="textarea" placeholder="Brief summary shown on project cards"></textarea>

                <!-- Long Description -->
                <label class="modal-label">Detailed Description</label>
                <textarea name="long_description" id="projectLongDescription" rows="4" class="textarea" placeholder="In-depth project description for the detail page"></textarea>

                <!-- Key Features -->
                <label class="modal-label">Key Features <span class="text-gray-500 text-xs">One per line</span></label>
                <textarea name="key_features" id="projectKeyFeatures" rows="3" class="textarea" placeholder="User authentication&#10;Real-time notifications&#10;Responsive design"></textarea>

                <!-- Challenges -->
                <label class="modal-label">Challenges & Solutions <span class="text-gray-500 text-xs">One per line</span></label>
                <textarea name="challenges" id="projectChallenges" rows="3" class="textarea" placeholder="Implemented caching to handle high traffic&#10;Used WebSocket for real-time updates"></textarea>

                <!-- Technologies Multi-select -->
                <label class="modal-label">Technologies Used</label>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <!-- Preview Link -->
                    <div>
                        <label class="modal-label">Preview Link</label>
                        <input type="url" name="preview_link" id="previewLink" class="input" placeholder="https://...">
                    </div>

                    <!-- Project Link -->
                    <div>
                        <label class="modal-label">Repository Link</label>
                        <input type="url" name="project_link" id="projectLink" class="input" placeholder="https://github.com/...">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <!-- Start Date -->
                    <div>
                        <label class="modal-label">Start Date <span class="text-pink-400 text-xs">Required</span></label>
                        <input type="date" name="start_date" id="startDate" class="input" required>
                    </div>

                    <!-- End Date -->
                    <div>
                        <label class="modal-label">End Date <span class="text-gray-500 text-xs">Leave empty if ongoing</span></label>
                        <input type="date" name="end_date" id="endDate" class="input">
                    </div>
                </div>

                <!-- Project Image -->
                <div>
                    <label class="modal-label">Project Image</label>
