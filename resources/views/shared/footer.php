 ...
 * Clean pixel artwork with flame and grass erased from base image (fire-pillar-base.png),
 * smooth swaying flame overlay (flame-only.png), swaying grass tuft (grass-two.png),
 * floating fire particle embers, blinking cursor banner, and social icon hover press.
 */
use app\core\View;
$data = View::getData();
$home = $data['home'] ?? [];
...
<footer id="contacts" class="footer-save-point">
  ...
  <div class="relative z-10 max-w-7xl mx-auto flex flex-col items-center gap-6 px-6">
    ...
    <!-- Social Link Square Boxes with Hover Press & Glow -->
    <div class="flex items-center justify-center gap-5 my-2">
      <?php if (!empty($github)): ?>
        <a href="<?= htmlspecialchars($github) ?>" target="_blank" class="social-icon-box w-16 h-16 bg-[#11162a] border-2 border-[#c8a951] flex items-center justify-center text-white no-underline shadow-md" title="GitHub">
          <svg class="w-7 h-7 fill-current" viewBox="0 0 496 512">
      ...
      <?php if (!empty($linkedin)): ?>
        <a href="<?= htmlspecialchars($linkedin) ?>" target="_blank" class="social-icon-box w-16 h-16 bg-[#11162a] border-2 border-[#c8a951] flex items-center justify-center text-white no-underline shadow-md" title="LinkedIn">
      ...
      <?php if (!empty($email)): ?>
        <a href="mailto:<?= htmlspecialchars($email) ?>" class="social-icon-box w-16 h-16 bg-[#11162a] border-2 border-[#c8a951] flex items-center justify-center text-white no-underline shadow-md" title="Email">
          <img src="<?= base_url('icons/mail.png') ?>" alt="Mail Icon" class="w-7 h-7 object-contain image-pixelated">
        </a>
      <?php endif; ?>
    </div>
